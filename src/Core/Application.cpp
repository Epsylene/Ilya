...
namespace Ilya 
{
    Application::Application(const WindowData& specification)
            : data(specification)
    {
        // Setup GLFW window
        ...
        window = glfwCreateWindow(data.width, data.height, data.name.c_str(), NULL, NULL);
...
//        m_LayerStack.clear();
    ...
    void Application::submit_ressource_free(std::function<void()>&& func)
    {
