...
namespace Ilya
{
    class Camera
    {
        public:
            ...
            Camera(Vec3 from, Vec3 at, Vec3 up, float aperture, float focus_dist, float fov, float aspect = 16.f/9.f, float t_open = 0.f, float t_close = 1.f):
                    aspect(aspect), lens(aperture / 2.f), t_open(t_open), t_close(t_close)
            {
                ...
                // the distance between the field of view cone and the
                // camera -plane axis at the point where the plane is
                // situated, which correspond geometrically to the tangent
                // of half the cone's aperture angle (that is, the FOV
                // angle). The rays are cast from the origin towards this
                // plane and form the image.
                ...
                // focal} because the Z axis points outwards the viewport
                // plane)
            }
