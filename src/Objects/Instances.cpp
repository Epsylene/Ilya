...
namespace Ilya
{
    bool Translate::hit(const Ray& r, float tmin, float tmax, HitRecord& rec) const
    {
        ...
        // which demands full information on the object's geometry),
        // two new Hittable classes are created, which take Hittable
        // instances are represent them translated and/or rotated.
        // That is, if one creates a Hittable A, then translation of
        // A is performed via A = Translate{A, ...}; A is still a
        // Hittable object, but of class Translate containing a pointer
        // to its actual shape (box, sphere, etc). Then, when hit
        // detection is performed, it is the Translate class hit function
        // that is called, which allow us to work on the incoming ray
        // instead of the actual object's geometry in order to do the
    ...
    bool Translate::bounding_box(float t0, float t1, BoundingBox& box) const
    {
    ...
    template<Axis axis>
    Rotate<axis>::Rotate(const std::shared_ptr<Hittable>& obj, float angle):
        obj(obj)
    {
        hadbox = obj->bounding_box(0.f, 1.f, box);
